...
function addToCartSuccess (jqXHR, textStatus, errorThrown){
  ...
  else if (_cartStyles == 'sidebar') {$('.cart-toggle').trigger('click');}
...
var AT_Main = {
    ...
    ,updateLayerCart : function(obj){
      ...
      if (typeof obj == 'object') {
        ...
        Shopify.getCart(function(cart) {
          ...
          AT_Main.currenciesFormat('#layer-cart-total span.money, .layer-addcart-modal .money');
        });
        ...
        setTimeout(function(){
          ...
          //   if ($('#header-style').data('style') == '6') {
          //     e   = $('#header-style .header-desktop .header-main > div:last-child');
          //     top = !$('#header-style .header-desktop').data('sticky') ? 0 : $(window).scrollTop() > 0 ? e.outerHeight() : e.outerHeight()+ $('#topbar').outerHeight();
          //   }else{
    ...
    ,toggleCartSidebar : function(){
      jQuery('.cart-toggle').on('click',function (e) {
        if (!jQuery('body').hasClass('templateCart')) {
          e.stopPropagation();
          if($(this).attr('href').includes('cart')){return;}
          AT_Main.fixNoScroll();
  ...
  ,newsletterCoupon: function(){ // Show coupon code when subscribe newsletter
    ...
    jQuery('#mc-button2').on('click', function (event) {
  ...
  ,init_handle : function(){
    ...
    $(document).on('click', '.currencies-list li', e=>{
      let _value = $(e.currentTarget).attr('data-value');
      $(`.shopify-currency-form option[data-value="${_value}"]`).prop('selected',true).trigger('change');
    })
...
jQuery(document).ready(function($) {
  ...
  i_sections.register('newsletter-popup',function(){
    ...
    $("#newsletter_popup").parents('.shopify-section').first().on('shopify:section:select', function(){$('#newsletter_popup').modal('show');});
    $("#newsletter_popup").parents('.shopify-section').first().on('shopify:section:deselect', function(){$('#newsletter_popup').modal('hide');});
  });
